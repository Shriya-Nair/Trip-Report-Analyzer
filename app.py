    ...
    if total_tat > 0 and not filtered_tat_df.empty:
        ...
        for i in range(len(filtered_tat_df)):
            ...
            for s in ["stage1", "stage2", "stage3", "stage4", "stage5"]:
                col = tat_columns[s]
                val = pd.to_numeric(filtered_tat_df[col].iloc[i], errors='coerce') if col and col in filtered_tat_df.columns else 0
                stage_vals.append(val if not pd.isna(val) else 0)
            
            total_time_val = sum(stage_vals)
            ...
            performance_data.append({
                ...
                "Icon": icon, "CSS Class": css_class
            })
        
        perf_df = pd.DataFrame(performance_data)
        perf_summary = perf_df.groupby(["Level (Arabic)", "Level (English)", "Icon", "CSS Class"]).agg(
            No_of_Trips=("Total Time (min)", "count"), Avg_Time=("Total Time (min)", "mean"),
            Min_Time=("Total Time (min)", "min"), Max_Time=("Total Time (min)", "max")
        ...
        for _, row in perf_summary.iterrows():
            ...
            table_html += f'<td><strong>{row["Level (Arabic)"]}</strong></td><td>{row["Level (English)"]}</td>'
            table_html += f'<td>{row["Icon"]}</td><td><strong>{row["No_of_Trips"]}</strong></td>'
            table_html += f'<td>{row["Avg_Time"]:.1f}</td><td>{row["Min_Time"]:.1f}</td><td>{row["Max_Time"]:.1f}</td><td>{pct:.1f}%</td>'
            table_html += '</tr>'
        table_html += '</tbody></table>'
        st.markdown(table_html, unsafe_allow_html=True)
